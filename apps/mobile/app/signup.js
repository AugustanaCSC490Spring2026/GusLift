...
export default function Signup() {
  ...
  // Kept from main: full role/setup-aware routing on stored session
  ...
  // Access denied screen — shown when non-Augustana email is used
  if (accessDenied) {
    return (
        ...
        {isGoogleClientMissing && (
          <View style={styles.warningBox}>
            ...
            <Text style={styles.warningText}>
              Google sign-in is not configured for {platformName}. Set {expectedClientEnvVar} in .env and restart Expo.
            </Text>
          </View>
        )}
        ...
        <TouchableOpacity
          ...
          onPress={async () => {
            if (isGoogleClientMissing) {
              Alert.alert("Configuration Error", `Missing ${expectedClientEnvVar} for ${platformName}.`);
              return;
            }
            if (!request) {
              Alert.alert("Not Ready", "Auth request is still initializing. Restart Expo with --clear if this persists.");
              return;
            }
            try {
              setLoading(true);
