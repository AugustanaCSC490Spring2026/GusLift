...
__name(decodeUserIdFromToken, "decodeUserIdFromToken");
...
async function fetchUserAndSchedule(env, userId) {
...
function normalizeLocation(residence) {
  if (!residence || !String(residence).trim())
    return "UNKNOWN";
  return String(residence).trim().toUpperCase().replace(/\s+/g, "_").replace(/[^A-Z0-9_]/g, "") || "UNKNOWN";
}
__name(normalizeLocation, "normalizeLocation");
var TIME_PATTERN = /^([01]\d|2[0-3]):[0-5]\d$/;
function resolveMatchingSlot(schedule, location) {
...
__name(resolveMatchingSlot, "resolveMatchingSlot");
function isDayKey(value) {
  return value === "mon" || value === "tue" || value === "wed" || value === "thu" || value === "fri" || value === "sat" || value === "sun";
}
__name(isDayKey, "isDayKey");
function resolveMatchingSlotWithOverride(schedule, residence, override) {
...
var MatchingRoom = class {
  ...
  handleRiderRequest(userId, ev) {
    if (ev.rider_id !== userId)
      return;
    const rider = { rider_id: ev.rider_id, joined_at: Date.now() };
